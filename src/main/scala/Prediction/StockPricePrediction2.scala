...
object StockPricePrediction2 {
    ...
    def prediction_data(spark: SparkSession,dfGpData:DataFrame,LabelCol:String): DataFrame ={
        ...
        val labeledData = assembledData.withColumn("next_open", lead("open", 22).over(Window.orderBy("timestamp")))
            .withColumn("next_high", lead("high", 22).over(Window.orderBy("timestamp")))
            .withColumn("next_low", lead("low", 22).over(Window.orderBy("timestamp")))
            .withColumn("next_close", lead("close", 22).over(Window.orderBy("timestamp")))
        ...
        val futureData = spark.sql(
            """
              |select
              |  date_add(timestamp, 30) timestamp,
              |  open,
              |  high,
              |  low,
              |  close,
